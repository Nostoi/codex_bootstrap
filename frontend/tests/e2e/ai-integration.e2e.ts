...
test.describe('AI Integration Features', () => {
  ...
  test('extracts tasks from natural language text', async ({ page }) => {
    ...
    await expect(suggestions.first().locator('[data-testid="suggested-priority"]')).toContainText('medium')
    await expect(suggestions.nth(1).locator('[data-testid="suggested-priority"]')).toContainText('low')
  })
  ...
  test('accepts and rejects task suggestions', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="task-list"]').getByText('Finish project documentation')).toBeVisible()
  ...
  test('provides AI-powered task improvement suggestions', async ({ page }) => {
    ...
    await expect(suggestionsList.getByText('Break down this task into smaller subtasks')).toBeVisible()
    await expect(suggestionsList.getByText('Consider scheduling this during high-energy periods')).toBeVisible()

    // Apply a suggestion
    await suggestionsList.getByRole('button', { name: /apply suggestion/i }).first().click()
...
test.describe('AI Integration Performance and Security', () => {
  ...
  test('handles sensitive data appropriately', async ({ page }) => {
    ...
    const suggestions = await aiPage.getSuggestedTasks()
    await expect(suggestions.getByText('Process payroll data for employee [REDACTED]')).toBeVisible()
  })
