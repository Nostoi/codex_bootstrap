...
export const Badge = React.forwardRef<HTMLSpanElement, BadgeProps>(
  ({ 
    ...
    const accessibleLabel = ariaLabel || generateAccessibleLabel({
      variant,
      energyLevel,
      status,
      confidence,
...
function generateAccessibleLabel({
  ...
  priority,
  children
}: Pick<BadgeProps, 'variant' | 'energyLevel' | 'status' | 'confidence' | 'priority' | 'children'>): string {
...
export const StatusBadge: React.FC<{ status: 'pending' | 'in-progress' | 'blocked' | 'done'; className?: string }> = ({ 
  status, 
  className 
}) => (
...
export const ConfidenceBadge: React.FC<{ confidence: 'high' | 'medium' | 'low'; className?: string }> = ({ 
  confidence, 
  className 
}) => (
