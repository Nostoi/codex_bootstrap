...
export default function TaskCard({ 
  ...
  const getUrgencyStyle = () => {
    ...
    if (task.hardDeadline) {
      const days = Math.ceil((new Date(task.hardDeadline).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {energyStyle && (
          ...
          >
            <span className="mr-1" aria-hidden="true">{energyStyle.icon}</span>
            {energyStyle.label}
          </span>
        )}
        ...
        {focusStyle && (
          ...
          >
            <span className="mr-1" aria-hidden="true">{focusStyle.icon}</span>
            {focusStyle.label}
          </span>
        )}
        ...
        {sourceStyle && (
          <span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-700">
            <span className="mr-1" aria-hidden="true">{sourceStyle.icon}</span>
            {sourceStyle.label}
          </span>
        )}
      </div>
