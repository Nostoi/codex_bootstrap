...
const testEndpoints = [
  ...
  { method: 'GET', path: '/auth/microsoft', description: 'Microsoft OAuth initiation' },
  {
    method: 'GET',
    path: '/auth/profile',
    description: 'Protected profile endpoint (requires JWT)',
  },
...
async function testEndpoint(endpoint) {
  return new Promise(resolve => {
    ...
    const req = http.request(options, res => {
      console.log(
        `✅ ${endpoint.method} ${endpoint.path} - Status: ${res.statusCode} (${endpoint.description})`
      );
    ...
    req.on('error', err => {
      console.log(
        `❌ ${endpoint.method} ${endpoint.path} - Error: ${err.message} (${endpoint.description})`
      );
...
async function runTests() {
  ...
  console.log(
    'Note: Some endpoints like /auth/profile will return 401 without valid JWT tokens, which is expected.'
  );
}
