...
describe("AiService", () => {
  ...
  beforeEach(() => {
    mockFetch = jest.fn().mockResolvedValue({
      json: () =>
        Promise.resolve({ choices: [{ message: { content: "resp" } }] }),
    });
  ...
  it("generates tasks", async () => {
    mockFetch.mockResolvedValueOnce({
      json: () =>
        Promise.resolve({ choices: [{ message: { content: "a\nb" } }] }),
    });
  ...
  it("summarizes text", async () => {
    mockFetch.mockResolvedValueOnce({
      json: () =>
        Promise.resolve({ choices: [{ message: { content: "sum" } }] }),
    });
  ...
  it("suggests next tasks", async () => {
    mockFetch.mockResolvedValueOnce({
      json: () =>
        Promise.resolve({ choices: [{ message: { content: "s1\ns2" } }] }),
    });
