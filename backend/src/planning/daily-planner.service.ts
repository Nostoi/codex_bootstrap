...
export class DailyPlannerService {
    ...
    if (!userSettings) {
      userSettings = await this.prisma.userSettings.create({
        data: {
          ...
          workStartTime: '09:00',
          workEndTime: '17:00',
          // focusSessionLength: 90, // Field not in schema - using default in code
          preferredFocusTypes: [],
        },
      });
    }
  ...
  private generateTimeSlots(
    ...
    const slotDuration = 90; // minutes - default focus session length (focusSessionLength field not in schema)
