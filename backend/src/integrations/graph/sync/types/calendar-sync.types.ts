...
export function isRemovedEvent(
  event: GraphCalendarEvent | GraphDeltaEvent
): event is RemovedGraphEvent {
  return '@removed' in event && event['@removed'] !== undefined && event['@removed'] !== null;
}

export function isActiveEvent(
  event: GraphCalendarEvent | GraphDeltaEvent
): event is ActiveGraphEvent {
  return !isRemovedEvent(event);
}

// Enhanced type guards that work with GraphCalendarEvent (which may have optional @removed)
export function isRemovedGraphCalendarEvent(
  event: GraphCalendarEvent | GraphDeltaEvent
): event is RemovedGraphEvent {
  return '@removed' in event && event['@removed'] !== undefined && event['@removed'] !== null;
}

export function isActiveGraphCalendarEvent(
  event: GraphCalendarEvent | GraphDeltaEvent
): event is GraphCalendarEvent & ActiveGraphEvent {
  return !isRemovedGraphCalendarEvent(event);
}

// Utility functions for safe '@removed' property access
export function getRemovedReason(event: GraphCalendarEvent | GraphDeltaEvent): string | undefined {
  return isRemovedEvent(event) ? event['@removed'].reason : undefined;
}

export function isEventDeleted(event: GraphCalendarEvent | GraphDeltaEvent): boolean {
