...
export class DeltaSyncManager {
  ...
  ): Promise<{ events: (GraphCalendarEvent | GraphDeltaEvent)[]; deltaToken?: string }> {
   ...
   */
  parseDeltaChanges(events: (GraphCalendarEvent | GraphDeltaEvent)[]): {
    created: (GraphCalendarEvent | GraphDeltaEvent)[];
    updated: (GraphCalendarEvent | GraphDeltaEvent)[];
    deleted: string[]; // Only IDs for deleted events
  } {
    const created: (GraphCalendarEvent | GraphDeltaEvent)[] = [];
    const updated: (GraphCalendarEvent | GraphDeltaEvent)[] = [];
  ...
  private filterDeltaEvents(
    events: GraphCalendarEvent[] | GraphDeltaEvent[],
    options: DeltaSyncOptions
  ): GraphCalendarEvent[] {
    let filteredEvents = events;

    if (options.skipDeleted) {
      filteredEvents = filteredEvents.filter(event => isActiveEvent(event)) as GraphCalendarEvent[];
    }

    return filteredEvents as GraphCalendarEvent[];
  }
